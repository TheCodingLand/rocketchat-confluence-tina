...
query_tina = (query, links) ->
    tina_url = nconf.get("TINA_PIPELINE_URL")
    data = JSON.stringify({query: query, links:links})
    msg.http(tina_url, {timeout: timeout}).headers(headers).post(data) (e, res, body) ->
    if e
      msg.reply "Error: #{e}"
      return
